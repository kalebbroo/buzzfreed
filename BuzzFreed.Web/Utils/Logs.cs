...
public static class Logs
{
    ...
    /// <summary>Whether log saving is enabled.</summary>
    ...
    public static void StartLogSaving(string logDirectory = "logs")
    {
        if (!SaveLogToFile)
        {
        ...
        string logFileName = $"buzzfreed-{time:yyyy-MM-dd_HH-mm-ss}-{Environment.ProcessId}.log";
        LogFilePath = Path.Combine(logDirectory, logFileName);

        Directory.CreateDirectory(Path.GetDirectoryName(LogFilePath) ?? logDirectory);
    ...
    public static void SaveLogsToFileOnce()
    {
        ...
        if (toStore.Length > 0 && !string.IsNullOrEmpty(LogFilePath))
        {
    ...
    public static void LogWithColor(ConsoleColor prefixBackground, ConsoleColor prefixForeground, string prefix, ConsoleColor messageBackground, ConsoleColor messageForeground, string message, LogLevel level)
    {
        lock (ConsoleLock)
        {
            ...
            if (Environment.TickCount64 - LastLogTime > RepeatTimestampAfter.TotalMilliseconds && LastLogTime != 0)
            {
                ...
                Console.WriteLine($"== BuzzFreed logs {timestamp:yyyy-MM-dd HH:mm} ==");
            }
